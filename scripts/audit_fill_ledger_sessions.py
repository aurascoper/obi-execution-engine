...
def main():
    ...
    print(
        f"# {sum(len(v) for v in fills_by_sym.values())} fills across "
        f"{len(fills_by_sym)} symbols",
        file=sys.stderr,
    )
    ...
    for w in windows:
        ...
        # Per-fill ρ
        rho_fill = (
            pearson(per_fill_ledger, per_fill_hl) if len(per_fill_ledger) >= 2 else None
        )
            ...
            rows.append(
                {
                    ...
                    "residual_vs_api": hl_pnl_api.get(s, 0.0)
                    - per_sym_ledger.get(s, 0.0),
                    "residual_vs_field": per_sym_hl.get(s, 0.0)
                    - per_sym_ledger.get(s, 0.0),
                }
            )
        ...
        print(
            f"  ρ per-fill (ledger vs hl.closed_pnl field): {rho_fill:+.4f}"
            if rho_fill is not None
            else "  ρ per-fill: N/A"
        )
        print(
            f"  ρ per-symbol (ledger vs HL API):           {rho_sym_vs_api:+.4f}"
            if rho_sym_vs_api is not None
            else "  ρ per-sym vs API: N/A"
        )
        print(
            f"  ρ per-symbol (ledger vs HL field sum):     {rho_sym_self:+.4f}"
            if rho_sym_self is not None
            else "  ρ per-sym self: N/A"
        )
        ...
        for sym in FOCUS_SYMS:
            ...
            if f is None:
                ...
                print(
                    f"    {sym:14s}  ledger=${f['ledger']:>+9.2f}  "
        ...
        for r in rows[:5]:
            print(
                f"    {r['sym']:14s}  ledger=${r['ledger']:>+9.2f}  "
    ...
    rho_7d = next(
        (s["rho_per_symbol_vs_hl_api"] for s in summaries if s["window_days"] == 7),
        None,
    )
    ...
    rules = [
        (
            "14d ledger-vs-HL ρ ≥ 0.95",
            rho_14d is not None and rho_14d >= 0.95,
            f"{rho_14d:+.4f}" if rho_14d is not None else "N/A",
        ),
        (
            "7d ledger-vs-HL ρ ≥ 0.95",
            rho_7d is not None and rho_7d >= 0.95,
    ...
    if zec_14d_focus is not None:
        ...
        prior_resid = -108.10
        reduction_pct = (
            (1 - abs(zec_resid) / abs(prior_resid)) * 100 if prior_resid != 0 else 0
        )
        rules.append(
            (
    ...
    if eth_14d_focus is not None:
        eth_sign_match = (eth_14d_focus["ledger"] >= 0) == (
            eth_14d_focus["hl_api"] >= 0
        )
        rules.append(
            (
                "ETH 14d sign matches HL",
                eth_sign_match,
    ...
    print()
    print(
        f"  → {'ACCEPT — Mode 2A established, Mode 2B build justified' if accepted else 'REJECT — fix accounting before Mode 2B'}"
    )
