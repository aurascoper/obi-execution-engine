...
|--------------------|:-:|:-:|-------|
| **§1 Mean-Reversion** — OU process, 60-day z-score, $s_\text{entry}=1.25\sigma$, $s_\text{exit}=0.50\sigma$ | ✓ Full | ✓ Partial | Crypto uses 60-**minute** window, not 60-day — intentional divergence for HF micro-structure |
| **§1 Short selling** — "sell short when $s_t > +s_\text{entry}$" | ✓ Implemented | ✗ Not applicable | Alpaca does not support crypto short selling; equities engine is bidirectional |
...
| **§3 Order-Book Imbalance** — $\rho_t$ formula, Cartea et al. (2018) | ✓ Full (NBBO) | ✓ Full (L2) | Crypto uses true L2 depth at 5 levels; equities synthesizes NBBO quotes to a single-level OBI |
...
$$\text{cap}(s) = \begin{cases} 1 & s \in \{\text{Defense},\, \text{Energy},\, \text{Energy ETF},\, \text{Nuclear Energy}\} \\ 2 & s = \text{Semiconductors} \\ 3 & \text{otherwise} \end{cases}$$
