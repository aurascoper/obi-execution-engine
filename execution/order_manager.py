...
class OrderManager:
    ...
    async def start_trade_updates(self) -> None:
        ...
        async def _on_trade_update(data) -> None:
            try:
                ...
                event = (
                    event_raw.value if hasattr(event_raw, "value") else str(event_raw)
                )
    ...
    async def submit_limit(
        ...
        """
        if not self._breaker.validate_order(
            symbol,
            qty,
            notional,
        ...
        try:
            return await self._submit_to_api(
                symbol, side, qty, limit_px, notional, tif, cid
            )
